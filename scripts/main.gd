...
func _show_battle_screen(result = {}):
	...
	if not battle_over:
		...
		if bool(current.get("sea_battle", false)):
			...
			var zone_band = GameData.sea_zone_level_band(str(current.get("sea_zone_id", "mediterranean")))
			content.add_child(_label("%s Lv.%d–%d海域｜装备掉落：%s" % [str(current.get("sea_zone_name", "未知海域")), int(zone_band.min), int(zone_band.max), str(current.get("loot_tier_name", "航海装备"))], 11, GOLD))
