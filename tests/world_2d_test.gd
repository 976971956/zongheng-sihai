...
func _run():
	...
	var battle_equipment = scene.battle_stage.player_model.equipment_visual_state()
	_check(str(battle_equipment.skin) == "warrior" and str(battle_equipment.weapon) == "warrior_blade" and bool(battle_equipment.has_weapon_layer), "战斗舞台角色必须穿上当前套装皮肤，并显示已装备的武器")
	scene.battle_stage.set_battle_values({"player_hp": scene.state.player.hp})
	battle_equipment = scene.battle_stage.player_model.equipment_visual_state()
	_check(str(battle_equipment.skin) == "warrior" and str(battle_equipment.weapon) == "warrior_blade", "战斗逐回合更新血量时不能丢失角色的装备外观")
