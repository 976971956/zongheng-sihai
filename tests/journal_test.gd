...
func _run():
	...
	_check(_has_text(scene.modal_layer, "扬帆拉古萨") and _has_text(scene.modal_layer, "400 经验"), "完整日志必须安全显示无道具奖励的任务")
	scene._close_modal()

	scene.state.inventory["ghost_card"] = 1
	scene._open_inventory()
	_check(_has_button(scene.modal_layer, "启用"), "完整日志背包必须允许启用怪物卡")
	...
	scene._open_harbor()
	_check(_has_text(scene.modal_layer, "商会推荐") and _has_text(scene.modal_layer, "商会委托") and _has_button(scene.modal_layer, "加固船体"), "完整日志的贸易页必须包含商路推荐、循环委托与船体改造")
